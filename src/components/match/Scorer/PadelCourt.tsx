  ...
  return (
    ...
    >
      <motion.div 
        ...
        className={`w-14 h-14 rounded-full border-2 flex items-center justify-center shadow-xl transition-all duration-500 ${isServer ? 'bg-primary border-white' : 'bg-white/10 border-white/30 backdrop-blur-md group-hover:bg-white/20'}`}
      >
        ...
        {isServer && (
          <motion.div 
            ...
            className="absolute -top-1 -right-1 w-6 h-6 bg-primary rounded-full flex items-center justify-center shadow-md border border-white"
          >
            <PadelBall className="w-4 h-4 text-white" />
          </motion.div>
        )}
      </motion.div>
      <span className="text-[10px] font-bold text-white drop-shadow-md whitespace-nowrap group-hover:text-primary-container transition-colors">{name}</span>
    </div>
  );
};
        ...
        <div className="grid grid-cols-4 md:grid-cols-9 gap-3 max-h-[60vh] overflow-y-auto pr-2 custom-scrollbar">
          {options.map((opt) => (
            <button
              ...
              className={`h-16 rounded-2xl font-black text-xl transition-all ${value === opt ? 'bg-primary-container text-on-primary-container shadow-lg scale-110' : 'bg-surface-container-low text-on-surface hover:bg-surface-container-high'}`}
            >
              {opt}
            </button>
          ))}
        ...
        {isTiebreak && (
          <div className="absolute top-4 left-1/2 -translate-x-1/2 z-20">
            <motion.div 
              ...
              className="px-4 py-1.5 bg-primary-container text-on-primary-container rounded-full text-[10px] font-black uppercase tracking-[0.25em] shadow-lg border border-primary/30"
            >
              Tiebreak
            </motion.div>
          </div>
          ...
          <div className="flex-1 relative">
            ...
            <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 pointer-events-auto">
              <button 
                ...
                disabled={isMaxed}
                className={`w-20 h-20 md:w-24 md:h-24 rounded-[2rem] flex items-center justify-center transition-all shadow-xl ${isMaxed ? 'bg-white/5 text-white/10 cursor-not-allowed shadow-none' : 'bg-primary-container text-on-primary-container hover:scale-110 active:scale-95 shadow-primary/30'}`}
              >
